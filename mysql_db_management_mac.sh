...
list_databases() {
    ...
    $MYSQL_CMD -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" -e "SELECT schema_name FROM information_schema.schemata WHERE schema_name NOT IN ($system_dbs);" -sN
}

check_if_database_contains_tables() {
    local db_name=$1
    local tables=$($MYSQL_CMD -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" -D"$db_name" -e 'SHOW TABLES;' | awk '{print $1}' | grep -v '^Tables_in_')
    if [ -z "$tables" ]; then
        return 1
    else
        return 0
...
drop_all_tables_forcefully() {
    local db_name=$1
    local tables=$($MYSQL_CMD -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" -D"$db_name" -e 'SHOW FULL TABLES WHERE Table_Type = "BASE TABLE";' | awk '{print $1}' | grep -v '^Tables')
    ...
    $MYSQL_CMD -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" -D"$db_name" -e "$drop_tables_sql"

    local remaining_tables=$($MYSQL_CMD -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" -D"$db_name" -e 'SHOW TABLES;' | awk '{ print $1}' | grep -v '^Tables_in_')
...
import_sql_file() {
    ...
    echo -ne "\n"
    $MYSQL_CMD -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" "$db_name" <"$DOWNLOADS_PATH/$sql_file"
    if [ $? -eq 0 ]; then
